        ...
        <header className="flex h-16 items-center justify-between border-b bg-white px-6">
          ...
          <div className="ml-auto flex items-center gap-4">
            <div className="text-sm">
              <div className="font-medium">{profile?.first_name || session.user.email}</div>
              <div className="text-gray-500">{session.user.email}</div>
            </div>
            <div className="h-8 w-8 rounded-full bg-orange-500 text-white flex items-center justify-center">
              {profile?.first_name ? profile.first_name[0].toUpperCase() : session.user.email[0].toUpperCase()}
            </div>
          </div>
        </header>
